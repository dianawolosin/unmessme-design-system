...
We use a strict **Component-Based Design** system for consistency and scalability.

- **Components (The Building Blocks):** `Button`, `Input`, `Tag`, `Surface`, `Dialog`.
- **Patterns (The Compositions):**
    - **Cards:** `ProblemCard` (composed of `Surface` + `Tag` + `Button`).
    - **Chips:** `Tag` (used for filtering and labeling).
    - **FAB:** `Button` (configured as `size="xl"`).
    - **Chat:** `ChatInput` (composed of `Input` + `Button`).
    - **Dialogs:** `ClarifyingDialog` (composed of `Dialog` + `Button`).
